...
Everything below lives in [`docs/store/`](store/) and is generated. Nothing here
is hand-made, so nothing drifts from the shipped UI or the shipped mark.
...
| Marquee tile | `promo-marquee-1400x560.png` | 1400×560, optional | n/a | `promo-tiles.js` |

The listing icon is a **separate upload from the package**: the manifest icons
are what the browser shows in the toolbar and on the extensions page, while both
stores have their own listing image field. Same mark, different field.

Screenshots are shot against fixture data — invented case numbers, never a real
