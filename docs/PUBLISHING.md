...
- **Category:** Productivity (closest fit on both stores at time of
  ...
  store).
- **Screenshots:** use [`docs/store/`](store/) — 1280×800, the size Chrome
  requires and Firefox accepts, generated by `node scripts/store-screenshots.js`
  against local fixture data (invented case numbers), never a real account.
  `docs/screenshots/` is the README set and is the wrong shape for a store
  listing.

  Regenerate both sets before submitting if the layout has changed since they
  were last taken — check `git log -1 -- docs/store` against the last release
  that touched `core/`. Store screenshots showing a layout the user will not
  see are a rejection risk and a trust problem.

  The backdrop behind the panel (`test/backdrop.html`) is a replica of the
  USCIS account page. Keep it recognisably a mock — do not reproduce the DHS
  seal or the "official website of the United States government" banner in a
  published listing image, since the listing text states plainly that this tool
  is not affiliated with or endorsed by USCIS.
...
> No remote code. Every line of JavaScript in this extension is bundled into
> the package at build time from three auditable source files in the repository
> this extension is built from (`core/uscis-codes.js`, `core/uscis-style.js`
> and `core/uscis-tracker-core.js`, concatenated in that order); nothing is
> fetched or evaluated at runtime, and the extension makes no requests other
> than the read-only my.uscis.gov API calls described above.
