...
(function () {
  ...
  function save(key, value) {
    try {
      ...
      // Best effort only — nothing we can do if storage is full/unavailable.
    }
  }
  ...
  // True when an endpoint answered successfully but had nothing to say.
  function isEmptyPayload(payload) {
    return !!(payload && payload.__empty);
  }
  ...
  var STYLE = [
    ...
    "  font-variant-numeric: tabular-nums;",
    "}",
    ".uscistr-root .uscistr-quiet-track { height: 4px; border-radius: var(--ust-r-full); background: var(--ust-track); overflow: hidden; }",
    ".uscistr-root .uscistr-quiet-fill { height: 100%; border-radius: var(--ust-r-full); background: var(--ust-text-3); }",
    ".uscistr-root .uscistr-banner-inline { margin: 0; }",
  ...
  function buildCaseCard(entry, ordered) {
    ...
    if (docsNoteText || floorText) card.appendChild(noteBox([docsNoteText, floorText]));
